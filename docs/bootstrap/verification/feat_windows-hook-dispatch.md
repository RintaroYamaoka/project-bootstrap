...
HUMAN | perf       | **Windows 実機 (Git Bash) で体感が快適になったか** — bench の数値と、実セッションで Edit/Bash が引っかからないこと | user の Windows 機で `bash scripts/bench-hooks.sh 20` を実行 + 実セッション体感 | human | 開発機は WSL2 でターゲット環境が手元に無い (境界 (c))。予測: 旧 数秒/call → 新 0.2-0.5s/call。**数値が予測と大きく違う場合は plugin cache が旧版のままの可能性を先に疑う** (下の HUMAN 行)
HUMAN | deploy     | plugin cache が新版に更新されて初めて効く (merge ≠ deployed、配備カバレッジ③)。本作業中も cache は 0.31.0 で 4 版遅れの gate が発火していた (heredoc 誤検知を実際に踏んだ) | 両マシン (WSL2 / Windows) で `/plugin` の版数 or plugin cache dir の版が 0.36.0 以上 | human | Windows 側でも plugin update が必要。更新しない限り本変更は 1ms も効かない
