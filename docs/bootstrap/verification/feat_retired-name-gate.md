...
PASS | unit        | 除外はコードコメント / test fixture を覆わない (dogfood で判明した射程) | 実 PR での誤検知の観測 | ai | 最初に実例の語 `typeNo` を probe に使ったところ、gate 自身のコメントと test fixture がヒットして赤の原因が probe に絞れなかった。除外は `*.md` / `docs/**` / `CHANGELOG*` / marker のみで、**コード内のコメントは覆わない**
PASS | unit        | 上記に対する対の逃げ道 (`bootstrap-retired-ok` の行単位 opt-out) が効き、かつ行単位に留まる | 実 hook / engine / doctor の判定 | ai | ADR 0021 決定⑤ として追加。engine 4 assert (token 一致 / 兄弟行は trip / 近縁 token は opt-out しない) + gate 2 assert (annotated 行は通す / 同 file の別行は block) + doctor 2 assert (annotated 行は数えない / 別 file の実残存は数える)。**逃げ道を用意しなかった場合の代替手段は「marker の行を消す」= gate 無効化と「scope-glob でパスごと除外」= 過剰に広い除外**で、どちらも設定ファイル内で不可視に起きる (決定① が塞いだ形の再来)
PASS | metamorphic | opt-out の 2 つの呼び出し口 (engine の scan / doctor の sweep) がそれぞれ実効な assert に守られているか | 注入した変異をテストが殺すか | ai | 変異⑤ (engine の suppression 呼び出しを削除) = engine/gate が kill・doctor は使わないので生存が正 / 変異⑥ (doctor の suppression 適用を `false` に潰す) = doctor が kill。**両呼び出し口それぞれに kill する assert がある**ことを確認 (片方だけだと「gate は通すが doctor は数える」ズレが無音で戻る)
