...
TODO | e2e         | 必須チェック (`hooks` / `verification-closed`) が実 PR で緑になる | GitHub Actions の実 check 結果 | human+ai | PR を上げてから実測して埋める。この行が TODO の間は `verification-closed` 自身が赤になる (= 自分の gate に自分で捕まる状態から始める)
