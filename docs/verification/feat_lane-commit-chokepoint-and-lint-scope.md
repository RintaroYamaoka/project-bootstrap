...
HUMAN| manual      | **仕様判断**: lint gate の信号を commit に絞った結果、「ツリーに既存 debt があっても、それに触らない commit は通る」ようになった。これは意図した緩和だが、gate の約束を変えている | 設計者 (単一 orchestrator) の意図 — repo にも AI にも無い | human | 出力モックを提示して merge 前に確認する (ADR 0018 に trade-off 明記、CI whole-tree lint が backstop)
