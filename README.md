...
| `hooks/hooks.json` | 16 hook を `plugin.json` 経由でデフォルト発火 (SessionStart 採用 audit / test 先行 / commit 前 lint+test / destructive git op / bulk-stage / cross-session WIP / 宣言 branch 直 push / lane 外編集 / active lane 中の main tree 未隔離 source 編集の block (ADR 0005 guard 2) / `wip_limit` 超過の `git worktree add` の block (guard 3) / 依存方向 edit+commit / sprint 発火判定 gate + reminder / レビューなき並列 lane branch merge の block + approve 後の検出スイート実行 (guard 1)) |
