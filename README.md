...
| `hooks/hooks.json` | 13 hook を `plugin.json` 経由でデフォルト発火 (SessionStart 採用 audit / test 先行 / commit 前 lint+test / destructive git op / bulk-stage / cross-session WIP / 宣言 branch 直 push / lane 外編集 / 依存方向 edit+commit / sprint 発火判定 gate + reminder) |
