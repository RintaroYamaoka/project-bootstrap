...
| `/plan` slash command | 探索 → 計画 → 提示の流れを 1 コマンドで強制 | 計画中 (Phase 2) |
| `test-writer` / `implementer` / `refactorer` subagent | TDD の Red / Green / Refactor を分離して担う | 計画中 (Phase 3) |
| `/red` `/green` `/refactor` slash command | TDD ループの primitive | 計画中 (Phase 3) |
| Hooks | 規律を機械的に強制 (テスト実行強制等) | 計画中 (Phase 5) |
| プラグイン marketplace 配布 | 各プロジェクトから簡単に install できる形 | 計画中 (Phase 4) |
...
Phase が進むと `agents/`, `commands/`, `hooks/`, `examples/` が追加される。
...
marketplace 配布が整う (Phase 4) までは、ローカル参照で使う。
詳細は [Claude Code 公式ドキュメント](https://code.claude.com/docs/en/plugins-reference) を参照。

```bash
# 都度ロード (検証用)
claude --plugin-dir /path/to/project-bootstrap

# またはローカルプラグインとして登録 (推奨)
# 公式ドキュメントの手順に従う
```
...
| 2 | `/plan` slash command (MVP) | 次 |
| 3 | TDD subagent 群 + slash command 群 (`test-writer` / `implementer` / `refactorer` / `/red` / `/green` / `/refactor`) | 計画中 |
| 4 | プラグイン marketplace 配布 | 計画中 |
| 5 | Hooks による規律の機械的強制 | 計画中 |
| 6 | TDD セッションログのサンプル (`examples/`) | 計画中 |
...
未定。Phase 4 (marketplace 配布時) に決定する。
